					...
					<div class="right-container" :style="`height: ${postHeight}px`">
						...
						<div class="form-container">
							...
							<b-button 	:disabled="comment === '' || loadingComment"
										...
										@click="onPostComment">Post</b-button>
						</div>
					</div>
				</div>
			</b-card>
...
export default {
	...
	methods: {
		getReactionRow(index) {
			let arr = [];
			for (let i = this.reactions.length * index / this.reactionRows; i < this.reactions.length * (index + 1) / this.reactionRows; i++)
		...
		refreshPost(id = null, showLoading = true) {
			return new Promise((resolve, reject) => {
				...
				network.get(`/posts/${id ? id : this.$route.params.postid}`, {
