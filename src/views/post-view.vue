<template>
	<div class="scroll-container">
                ...
                <div class="post-container">
                    <div class="post">
                        ...
                        <div class="card-image">
                            <b-card-img ref="image" v-viewer="{ navbar: false }" :src="post.image ? `${IMG_URL}/${post.image.id}` : ''" @load="computePostHeight"/>
                    ...
                    <div class="right-container" :style="`height: ${postHeight}px`">
                        ...
                        <div class="comments-container">
                            <div class="comment" :key="comment.id" v-for="comment in post.comments">
                                <a :href="`#/profile/${post.user.id}`"><strong><i>{{comment.username}}</i></strong></a>&nbsp;&nbsp;
                                {{comment.comment}}
                                <div class="hint">{{comment.timePosted}}</div>
                            </div>
                        </div>
                        <div class="form-container">
                            <b-textarea id="comment" v-model="comment" placeholder="Add a comment..." no-resize=""/>
                            <b-button :disabled="comment === '' || loadingComment"
                                style="align-self: center" 
                                variant="outline-primary" @click="onPostComment">Post</b-button>
                        </div>
                    </div>
                </div>
            </b-card>
        ...
        <div class="content h-100" v-else>
            <p><i style="font-size: 1.3em; opacity: 0.6">This post does not exist.</i></p>
        </div>
	</div>
</template>
    ...
    methods: {
        ...
        refreshPost(id = null, showLoading = true) {
            return new Promise((resolve, reject) => {
                ...
                network.get(`/posts/${id ? id : this.$route.params.postid}`, { withCredentials: true })
                    .then(res => {
                        this.post = res.data;
        ...
        onPostComment() {
            ...
            .catch(err => {
                this.$swal({
                    ...
                    text: err.response && err.response.data ? err.response.data.error : 'Could not post the specified comment.',
                    icon: 'error'
                });
                this.loadingComment = false;
            });
        ...
        computePostHeight() {
            this.postHeight = Math.max((this.post.image.caption !== '' ? this.$refs.caption.offsetHeight : 0) + this.$refs.image.offsetHeight, 600);
            this.$forceUpdate();
        }
    },
    watch: {
